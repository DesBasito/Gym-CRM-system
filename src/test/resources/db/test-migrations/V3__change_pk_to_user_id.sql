...
WHERE EXISTS (SELECT 1 FROM trainers WHERE id = tt.trainer_id);
UPDATE trainers_trainees tt
SET trainee_id = (SELECT user_id FROM trainees WHERE id = tt.trainee_id)
WHERE EXISTS (SELECT 1 FROM trainees WHERE id = tt.trainee_id);


UPDATE trainings t
SET trainee_id = (SELECT user_id FROM trainees WHERE id = t.trainee_id)
WHERE EXISTS (SELECT 1 FROM trainees WHERE id = t.trainee_id);
UPDATE trainings t
SET trainer_id = (SELECT user_id FROM trainers WHERE id = t.trainer_id)
WHERE EXISTS (SELECT 1 FROM trainers WHERE id = t.trainer_id);
